...
float getTemperature(AnalogIn& analogIn) {     // from the TMP36 datasheet
	int adc_val = analogIn.readAdcSample();
   float cur_voltage = adc_val * (1.80f/4096.0f); // Vcc = 1.8V, 12-bit
...
int main(int argc, char* argv[]) {
   ...
   AnalogIn analogSensor(ADC);
   sprintf(str_payload, "%f", getTemperature(analogSensor));
